...
const FeaturedRaceCard = ({ race, type }) => {
    ...
    return (
        <div className={`race-card flex flex-col bg-white rounded-lg shadow-lg p-5 h-full ${type === 'next' ? 'border-2 border-[#e10600] ring-4 ring-red-100' : ''}`}>
            ...
            <div className="mt-auto">
                {type === 'previous' && race.podium ? (
                    ...
                    <>
                        ...
                        <p>{new Date(race.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric', timeZone: 'UTC' })}</p>
                    </>
                )}
            </div>
        </div>
...
const FullCalendar = ({ allRaces, isOpen }) => (
    <div className={`transition-all duration-700 ease-in-out overflow-hidden ${isOpen ? 'max-h-[1500px]' : 'max-h-0'}`}>
        <div className="bg-white rounded-b-lg shadow-md -mt-1 p-4 md:p-6 grid md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
            {allRaces.map(race => (
                <div key={race.round} className="flex items-center">
                    <span className="text-[#e10600] font-bold w-8">{race.round}.</span>
                    <span>{race.name} - {new Date(race.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', timeZone: 'UTC' })}</span>
                </div>
            ))}
        </div>
    </div>
  ...
  return (
    <div className="bg-[#f0f2f5] font-sans">
      ...
      <main className="container mx-auto p-4 md:p-6">
        ...
        <div onClick={() => setIsOpen(!isOpen)} className="bg-white rounded-lg shadow-md p-4 flex justify-between items-center cursor-pointer hover:shadow-lg transition-shadow">
  ...
  try {
    ...
    const allRaces = calendarData.MRData.RaceTable.Races.map(race => {
        ...
        return {
            ...
            date: race.date,
            podium: result ? { p1: result.Results[0].Driver.code } : null,
        };
    });
